...
#include "graphic/renderer/majercik2022_bidirectional_path_tracer.h"
...
Renderer* getRenderer(const std::string& name, Device* device) {
	...
	if (name == "Majercik2022BidirectionalPathTracer") return new Majercik2022BidirectionalPathTracer(device);
...
void screenshot(GraphicsEngine* engine, const std::string& rendererName) {
	...
	std::string path = std::string(BASE_PATH) + rendererName + "_0.ppm";
	for (unsigned int i = 1; std::filesystem::exists(path); ++i) {
		path = std::string(BASE_PATH) + rendererName + "_" + std::to_string(i) + ".ppm";
	}
	engine->saveLatestImage(path);
	std::cout << "Saved screenshot " << path << std::endl;
}

void saveCamera(FreeInput* input, const std::string& rendererName) {
	const std::string BASE_PATH = "res/camera/autogenerated/";
	std::string path = std::string(BASE_PATH) + rendererName + "_0.camera";
	for (unsigned int i = 1; std::filesystem::exists(path); ++i) {
		path = std::string(BASE_PATH) + rendererName + "_" + std::to_string(i) + ".camera";
	}
	input->saveCamera(path);
	std::cout << "Saved camera " << path << std::endl;
}
